  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/40 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="card mt-8 w-full max-w-2xl rounded-2xl p-6" onClick={(e) => e.stopPropagation()}>
        ...
        {error && <div className="py-6 text-center text-sm text-[var(--loss)]">{error}</div>}
          ...
          <>
            <div className="flex items-start justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={onClose} className="rounded-lg p-1.5 text-slate-400 hover:bg-[var(--surface-hover)] hover:text-[var(--ink-secondary)]"><X size={18} /></button>
              </div>
            </div>

            {/* Outcome banner */}
            {o && (
              <div className={`mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl p-3 ${o.result === "target" ? "bg-[var(--profit-dim)]" : o.result === "stop" ? "bg-[var(--loss-dim)]" : "bg-[var(--surface-raised)]"}`}>
                <span className={`text-sm font-semibold ${o.result === "target" ? "text-[var(--profit)]" : o.result === "stop" ? "text-[var(--loss)]" : "text-[var(--ink-secondary)]"}`}>
                  Outcome: {o.result}
                </span>
                <div className="flex items-center gap-4 text-xs text-[var(--ink-secondary)]">
                  <span>P&L <span className={`font-semibold ${(pnlPct ?? 0) >= 0 ? "text-[var(--profit)]" : "text-[var(--loss)]"}`}>{pnlPct != null ? `${pnlPct > 0 ? "+" : ""}${pnlPct.toFixed(2)}%` : "—"}</span></span>
                  <span>MFE <span className="text-[var(--profit)]">{o.mfe != null ? `${(o.mfe * 100).toFixed(1)}%` : "—"}</span></span>
                  <span>MAE <span className="text-[var(--loss)]">{o.mae != null ? `${(o.mae * 100).toFixed(1)}%` : "—"}</span></span>
            ...
            <div className="mt-4">
              <div className="mb-2 flex items-center justify-between text-xs font-semibold uppercase tracking-wide text-slate-400">
                ...
                <span className="flex gap-2 normal-case text-[var(--ink-muted)]">{t.reward_risk != null && <span className="rounded bg-[var(--profit-dim)] px-1.5 py-0.5 font-medium text-[var(--profit)]">{t.reward_risk}R</span>}{t.size_pct != null && <span>risk {t.size_pct}%</span>}</span>
              </div>
              ...
              {t.invalidation && <div className="mt-2 text-[11px] text-[var(--ink-muted)]">Invalidation: <span className="font-medium text-[var(--ink-secondary)]">{t.invalidation}</span></div>}
            </div>
            ...
            {t.psychology && t.psychology !== "no crowd extreme" && (
              <div className="mt-3 flex items-start gap-2 rounded-xl border border-[var(--accent)]/25 bg-[var(--accent-dim)] p-3 text-sm text-[var(--ink-secondary)]">
                <Brain size={15} className="mt-0.5 text-[var(--accent-bright)]" /><span><span className="font-medium text-[var(--accent-bright)]">Positioning:</span> {t.psychology}</span>
              </div>
            )}

            {t.rationale && (
              <div className="mt-3 rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] p-3">
                <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">AI thesis</div>
                <p className="text-sm leading-relaxed text-[var(--ink-secondary)]">{t.rationale}</p>
              </div>
            )}

            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-[var(--line)] pt-3 text-[11px] text-[var(--ink-muted)]">
              {t.price != null && <span>signal price: <span className="font-medium text-[var(--ink-secondary)]">{num(t.price)}</span></span>}
              {t.atr != null && <span>ATR: <span className="font-medium text-[var(--ink-secondary)]">{num(t.atr)}</span></span>}
              {o?.resolved_at && <span>resolved: <span className="font-medium text-[var(--ink-secondary)]">{new Date(o.resolved_at).toLocaleString()}</span></span>}
            </div>
          </>
        )}
      </div>
...
function Tile({ icon, label, value, tone }: { icon: React.ReactNode; label: string; value: number | null | undefined; tone?: "risk" | "reward" }) {
  const c = tone === "risk" ? "text-[var(--loss)]" : tone === "reward" ? "text-[var(--profit)]" : "text-[var(--ink)]";
  return (
    <div className="rounded-lg border border-[var(--line)] bg-[var(--surface-raised)] p-2.5">
...
function Stat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-[var(--line)] bg-[var(--surface-raised)] p-2">
      <div className="text-[10px] uppercase tracking-wide text-slate-400">{label}</div>
      <div className="text-sm font-semibold tabular-nums text-[var(--ink)]">{value}</div>
    </div>
  );
}
