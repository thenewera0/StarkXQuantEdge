  ...
  return (
    <Card className="card-pad">
      ...
      <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-gradient-to-br from-white/[0.04] to-transparent p-5">
        <div className="text-[11px] uppercase tracking-wide text-[var(--ink-muted)]">Combined (realized + floating)</div>
      ...
      <div className="mt-3 space-y-2">
        {strategies.map(([name, s]) => (
          <div key={name} className="flex items-center gap-3 rounded-xl border surface-raised p-3">
            <span className={`flex h-7 w-7 items-center justify-center rounded-lg ${s.paper ? "bg-[var(--warn-dim)]0/10" : "bg-[var(--accent)]/15"}`}>
              {s.paper ? <Zap size={13} className="text-[var(--warn)]" /> : <Wallet size={13} className="text-[var(--accent-bright)]" />}
            </span>
            <div className="min-w-0">
              <div className="flex items-center gap-2 text-sm font-medium capitalize text-white">
                {name.replace("(paper)", "")}
                {s.paper && <span className="rounded bg-[var(--warn-dim)]0/10 px-1.5 py-0.5 text-[9px] font-bold text-[var(--warn)] border border-amber-500/20">PAPER</span>}
              </div>
      ...
      <p className="mt-2.5 text-[10.5px] leading-relaxed text-[var(--ink-muted)]">
