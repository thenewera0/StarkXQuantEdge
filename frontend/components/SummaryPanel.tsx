    ...
    <Card className="card-pad">
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <CalendarDays size={16} className="text-[var(--accent-bright)]" />
      ...
      {s?.risk_state && (s.risk_state.circuit_halted || s.risk_state.drifting) && (
        <div className={`mb-3 flex items-center gap-2 rounded-xl border p-3 text-sm ${
          s.risk_state.circuit_halted ? "border-rose-200 bg-[var(--loss-dim)] text-[var(--loss)]" : "border-amber-200 bg-[var(--warn-dim)] text-amber-700"}`}>
          <span className={`h-2 w-2 rounded-full ${s.risk_state.circuit_halted ? "bg-[var(--loss-dim)]0" : "bg-[var(--warn-dim)]0"} animate-pulse`} />
          ...
          <div className="space-y-1.5">
            {Object.entries(s.allocator.weights).map(([fam, w]) => (
              <div key={fam} className="flex items-center gap-2 text-sm">
                <span className="w-24 shrink-0 capitalize text-[var(--ink-secondary)]">{fam}</span>
                ...
                <span className="w-10 shrink-0 text-right font-semibold tabular-nums text-[var(--ink-secondary)]">{Math.round(w * 100)}%</span>
      ...
      {L && (
        <div className="mt-5">
          ...
          <div className="rounded-xl border border-[var(--accent)]/25 bg-[var(--accent-dim)] p-3 text-sm text-[var(--ink-secondary)]">
            Trading is now restricted to regimes with proven positive expectancy:{" "}
            {L.tradeable_regimes.map((r) => (
              <span key={r} className="mr-1 inline-flex items-center gap-1 rounded bg-[var(--profit-dim)] px-1.5 py-0.5 text-xs font-medium text-[var(--profit)]">
                <CheckCircle2 size={11} /> {REGIME_LABEL[r] ?? r}
              </span>
            ))}
            . Excluded (proven-losing or unproven):{" "}
            {L.excluded_regimes.map((r) => (
              <span key={r} className="mr-1 inline-flex items-center gap-1 rounded bg-[var(--surface-hover)] px-1.5 py-0.5 text-xs text-[var(--ink-muted)]">
                  ...
                  return (
                    <div key={d} className="flex items-center justify-between text-sm">
                      <span className="flex items-center gap-2">
                        <span className={`h-1.5 w-1.5 rounded-full ${on ? "bg-[var(--profit-dim)]0" : "bg-rose-400"}`} />
                        <span className="font-medium capitalize text-[var(--ink-secondary)]">{d}</span>
          ...
          {L.paused_symbols && L.paused_symbols.length > 0 && (
            <div className="mt-3 rounded-xl border border-[var(--loss)]/25 bg-[var(--loss-dim)] p-3 text-sm text-[var(--ink-secondary)]">
              <span className="font-medium text-[var(--loss)]">Paused symbols</span> (proven-losing, auto-re-tested):{" "}
              {L.paused_symbols.map((s) => (
                <span key={s} className="mr-1 inline-block rounded bg-[var(--loss-dim)] px-1.5 py-0.5 text-xs font-medium text-[var(--loss)]">{s}</span>
              ))}
            </div>
          )}
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {Object.entries(L.regime_performance).map(([r, p]) => (
              <div key={r} className={`flex items-center justify-between rounded-lg border p-2.5 text-sm ${p.tradeable ? "border-[var(--profit)]/25 bg-white" : "border-[var(--line)] bg-[var(--surface-raised)]"}`}>
                <div className="flex items-center gap-2">
                  <span className={`h-1.5 w-1.5 rounded-full ${p.tradeable ? "bg-[var(--profit-dim)]0" : "bg-slate-300"}`} />
                  <span className="font-medium text-[var(--ink-secondary)]">{REGIME_LABEL[r] ?? r}</span>
  ...
  return (
    <div className="rounded-xl border border-[var(--line)] bg-[var(--surface-raised)] p-4">
      ...
      <div className="mt-1 flex gap-3 text-[11px] text-slate-400">
        <span>best <span className="text-[var(--profit)]">{usd(w.best_usd)}</span></span>
        <span>worst <span className="text-[var(--loss)]">{usd(w.worst_usd)}</span></span>
      </div>
    </div>
  );
}
