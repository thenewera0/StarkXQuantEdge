"""FLASH BOT 100-STRATEGY SWEEP — Systematic High-Frequency Strategy Search.

Run: python -m scripts.research_flash_100

Evaluates 100+ distinct high-frequency strategies across 14 liquid crypto pairs on 15m and 1h bars
with strict causal next-bar fills and per-market round-trip transaction costs.
"""
...
def load_dataset() -> dict[str, dict[str, pd.DataFrame]]:
    ...
    for itv in INTERVALS:
        ...
        for s in SYMBOLS:
            try:
                df = fetch_klines_history(s, itv, BARS)
...
# Strategy Configuration Grid Definitions
        ...
        for s, ind in dataset[itv].items():
            ...
            while i < n - 1:
                ...
                if trig == "mom_burst":
                    ...
                    if rsi and rsi < 38 and htf_long:
                        direction = "long"
                    elif bias == "long_short" and rsi and rsi > 62 and htf_short:
                        direction = "short"
                    ...
                    if cvd_z and cvd_z > 0.5 and htf_long:
                        direction = "long"
                    elif bias == "long_short" and cvd_z and cvd_z < -0.5 and htf_short:
                        direction = "short"
                ...
                # Trade execution (next-bar open fill)
                entry = opens[i + 1]
                stop_dist = 1.5 * atr
...
def main() -> None:
    print("Loading 15m & 1h Klines for Flash Bot 100-Strategy Sweep...")
    dataset = load_dataset()
    print("Dataset loaded. Building and evaluating 120 Flash Bot Strategy Configurations...\n")

    grid = list(itertools.product(TRIGGERS, HTF_FILTERS, BIASES, RISK_REWARDS, HOLD_BARS))
    print(f"Total strategy combinations generated: {len(grid)}")
    ...
    print("=" * 110)
    print(f"{'STRATEGY CONFIGURATION':50} {'TRADES':>7} {'WIN RATE':>9} {'TOTAL NET%':>12} {'EXP%/TRADE':>12} {'PF':>7}")
    print("=" * 110)
    for r in results[:20]:  # Top 20 winners
