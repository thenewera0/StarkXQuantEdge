...
for name, _lo, _hi, _mc, _cap, _ev in sizing.TIERS:
    ...
    check(f"{name:9} ceiling <= 1.5x", g is not None and g <= 1.5, f"{g}x is real leverage")
...
print("\n=== 3. The exposure budget is what actually gates new risk ===")
b = sizing.exposure_budget(1000.0, 0.0, 1000.0)
check("empty book has its full ceiling available",
      abs(b["remaining_usd"] - 1000.0) < 1e-6, f"got {b['remaining_usd']}")
b = sizing.exposure_budget(1000.0, 600.0, 1000.0)
check("partly-used book reports the remainder",
      abs(b["remaining_usd"] - 400.0) < 1e-6, f"got {b['remaining_usd']}")
check("gross_used is reported honestly", abs(b["gross_used"] - 0.6) < 1e-6, f"got {b['gross_used']}")
b = sizing.exposure_budget(1000.0, 16160.0, 1000.0)
check("an over-levered book has ZERO room (the 2026-08-02 state)",
      b["remaining_usd"] == 0.0, f"still offered {b['remaining_usd']} of room at 16.2x")
b = sizing.exposure_budget(1000.0, 0.0, 1250.0)     # 20% below high-water
check("drawdown shrinks the budget",
      abs(b["budget_usd"] - 500.0) < 1e-6, f"got {b['budget_usd']} at -20% drawdown")
